...
<Tabs>
  <Tab title="Polymarket">
    ...
    Find the `token_id` in the `outcomes` array for the side you want to trade:
    ...
    ```json
    {
      "markets": [{
        ...
        "outcomes": [
          { "label": "Yes", "token_id": "71321045679252..." },
          { "label": "No", "token_id": "48331043336612..." }
        ]
      }]
    }
    ```
...
<Tabs>
  <Tab title="Polymarket">
    ...
    <Note>
    **Limit orders** sit on the orderbook until matched. Use `"type": "market"` for immediate execution at best available price (don't include `price` field).
    </Note>
  </Tab>
  <Tab title="Kalshi">
    ```python Python
...
<CardGroup cols={2}>
  <Card title="Place Order" icon="paper-plane" href="/trading-api/trading/place-order">
    Learn about order types and constraints
  </Card>
...
token_id = market["outcomes"][0]["token_id"]  # Buy "Yes" side
