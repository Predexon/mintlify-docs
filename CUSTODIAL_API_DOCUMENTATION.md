...
| `POST` | `/api/users/:userId/kalshi/airdrop-sol` | Airdrop SOL for Kalshi fees |
...
| `solanaWalletAddress` | string | Solana wallet address for Kalshi deposits |
...
| `tokenId` | string | Polymarket only | The outcome token ID |
| `ticker` | string | Kalshi only | Market ticker (e.g., "PRES-2024-DJT") |
| `outcome` | string | Kalshi only | `Yes` or `No` |
| `side` | string | Yes | `buy` or `sell` |
| `amount` | string | Yes | USDC amount to spend (buy) or tokens to sell (sell) |
| `price` | string | Polymarket limit orders | Price per share (0-1) |
...
| Status | Error | Cause |
|--------|-------|-------|
| 400 | "Missing required fields: venue, tokenId, side, amount" | Missing required parameters |
...
| 500 | "Failed to cancel order" | Order not found or already filled |
...
| 500 | "Withdrawal failed" | Insufficient balance or network error |
...
| 400 | "Invalid wallet address format" | Wallet is not a valid Ethereum address |
| 500 | "Failed to get deposit info" | Bridge API error |
...
Airdrops SOL from the relayer to the user's Kalshi wallet for transaction fees.

- **Method:** `POST`
- **Path:** `/api/users/:userId/kalshi/airdrop-sol`
- **Auth Required:** Yes (must own user)
- **Geo-Restriction:** Blocked in Kalshi-restricted countries
...
| Status | Error | Cause |
|--------|-------|-------|
| 404 | "User does not have a Kalshi wallet" | Wallet not found |
| 500 | "Failed to airdrop SOL" | Relayer has insufficient SOL |
...
interface UserInfo {
  ...
  solanaWalletAddress: string | null;      // Solana wallet address
}
```
...
1. **Active:** Market is trading, position can be closed via sell order
2. **Resolved:** Market has settled, outcome determined
3. **Redeemable:** Position can be redeemed for USDC

### Fee Structure

- **Polymarket:** Trading fees per Polymarket's CLOB fee schedule
- **Kalshi:** DFlow swap fees included in execution
- **Gas Fees:**
  - Polymarket: Gasless relay (no user gas needed)
  - Kalshi: User pays SOL for transaction fees (configurable via relayer)
